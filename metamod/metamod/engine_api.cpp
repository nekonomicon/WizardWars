...
unsigned int mm_FunctionFromName( const char *pName ) {
	META_ENGINE_HANDLE(unsigned int, 0, FN_FUNCTIONFROMNAME, pfnFunctionFromName, (pName));
	RETURN_API()
}
const char *mm_NameForFunction( unsigned int function ) {
...
int mm_RandomLong(int lLow, int lHigh) {
	META_ENGINE_HANDLE(int, 0, FN_RANDOMLONG, pfnRandomLong, (lLow, lHigh));
	RETURN_API()
}
