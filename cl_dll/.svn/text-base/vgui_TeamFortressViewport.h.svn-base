...
public:
	...
	{
		if( m_iPlayerClass != 0 && g_iPlayerClass != m_iPlayerClass )
			return true;
...
//WIZWARS
	CMenuPanel*	 ShowKickMenu( void );
	void		 CreateKickMenu( void );
	CMenuPanel*	 ShowWorldPanel( void );
	void		 CreateWorldPanel( void );
...
public:
	...
	int	GetValidTeams() { return m_iValidTeams; };
	...
	int MsgFunc_ValTeams(const char *pszName, int iSize, void *pbuf );
...
// WIZWARS
	CKickMenu *		m_pKickMenu;
	CWorldPanel	*	m_pWorldPanel;
	CustomHUD *		m_pCustomHUD;
};
...
public:
	...
	virtual int IsNotValid()
	{
		...
		if( !( gViewPort->GetValidTeams() & ( 1 << ( m_iTeamNumber - 1 ) ) ) )
			return true;
		...
		if( m_iTeamNumber != g_iTeamNumber )
			return false;
...
// WIZWARS
	virtual void setTransparency(int iTrans)
	{
		m_iTransparency=iTrans;
	}
};
...
class CKickMenu : public CMenuPanel{
public:
	CKickMenu(int iTrans, int iRemoveMe, int x,int y,int wide,int tall);
	virtual void Open( void );
	virtual void Update();
	virtual void Reset( void ){
		CMenuPanel::Reset();
...
private:
	Label *m_pLabel;
	CommandButton *m_pButtons[MAX_PLAYERS];

	int m_X,m_Y,m_SizeX,m_SizeY;
};

class CWorldPanel : public CMenuPanel{
public:
	CWorldPanel(int iTrans, int iRemoveMe, int x,int y,int wide,int tall);
	virtual void Open( void );
	virtual void Update();
	virtual void Reset( void ){
		CMenuPanel::Reset();
...
private:
	Label *m_pLabel;
	ImagePanel *m_pWorldPanel;
	ImagePanel *m_pCrossPanel;
	CommandButton *m_pButton;
	CTransparentPanel *m_pPanel1;
	CTransparentPanel *m_pPanel2;
