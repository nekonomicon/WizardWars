...
CSchemeManager::CSchemeManager( int xRes, int yRes )
{
	...
	char token[1024];
	char *pFile = (char*)LoadFileByResolution( "schemes/", xRes, ".txt" );
	m_xRes = xRes;
	...
	if ( !pFile )
	{
		gEngfuncs.Con_DPrintf( "Unable to find schemes/*.txt\n");
		goto buildDefaultFont;
	}

	// record what has been entered so we can create defaults from the different values
	bool hasFgColor, hasBgColor, hasArmedFgColor, hasArmedBgColor, hasMouseDownFgColor, hasMouseDownBgColor, hasBorderColor;
	...
	while ( strlen(token) > 0 && (currentScheme < numTmpSchemes) )
	{
		...
		if ( !stricmp(paramName, "SchemeName") )
		{
			...
			if ( pScheme )
			{
				...
				if( !hasBorderColor )
				{
					memcpy( pScheme->borderColor, pScheme->fgColor, sizeof(pScheme->borderColor) );
				}
