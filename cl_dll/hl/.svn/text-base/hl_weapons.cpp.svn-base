...
WWStaff					g_Staff;
WWShieldSpell			g_ShieldSpell;
WWSealSpell				g_SealSpell;
WWMissileSpell			g_MissileSpell;
WWDoubleMissileSpell	g_DoubleMissileSpell;
WWUpdraftSpell			g_UpdraftSpell;
WWLevitateSpell			g_LevitateSpell;
WWLightningBoltSpell	g_LightningBoltSpell;
WWSpotBoltSpell			g_SpotBoltSpell;
WWFlameLickSpell		g_FlameLickSpell;
WWFireBallSpell			g_FireBallSpell;
WWDeathRaySpell			g_DeathRaySpell;
WWSkullSpell			g_SkullSpell;
WWWhiteRaySpell			g_WhiteRaySpell;
WWForceBlastSpell		g_ForceBlastSpell;
WWPebbleBlastSpell		g_PebbleBlastSpell;
WWRollingStoneSpell		g_RollingStoneSpell;
WWBirdSpell				g_BirdSpell;
WWBearBite				g_BearBite;
WWDragonBreathSpell		g_DragonBreathSpell;
WWWyvernSpell			g_WyvernSpell;
WWIcePokeSpell			g_IcePokeSpell;
WWFreezeRaySpell		g_FreezeRaySpell;
WWBeanStalkSpell		g_BeanStalkSpell;
WWThornBlastSpell		g_ThornBlastSpell;
CComboSpell				g_ComboSpell;
...
void HUD_InitClientWeapons( void )
{
	...
	HUD_PrepEntity( &g_Staff,				&player );
	HUD_PrepEntity( &g_ShieldSpell,			&player );
	HUD_PrepEntity( &g_SealSpell,			&player );
	HUD_PrepEntity( &g_MissileSpell,		&player );
	HUD_PrepEntity( &g_DoubleMissileSpell,	&player );
	HUD_PrepEntity( &g_UpdraftSpell,		&player );
	HUD_PrepEntity( &g_LevitateSpell,		&player );
	HUD_PrepEntity( &g_LightningBoltSpell,	&player );
	HUD_PrepEntity( &g_SpotBoltSpell,		&player );
	HUD_PrepEntity( &g_FlameLickSpell,		&player );
	HUD_PrepEntity( &g_FireBallSpell,		&player );
	HUD_PrepEntity( &g_DeathRaySpell,		&player );
	HUD_PrepEntity( &g_SkullSpell,			&player );
	HUD_PrepEntity( &g_WhiteRaySpell,		&player );
	HUD_PrepEntity( &g_ForceBlastSpell,		&player );
	HUD_PrepEntity( &g_PebbleBlastSpell,	&player );
	HUD_PrepEntity( &g_RollingStoneSpell,	&player );
	HUD_PrepEntity( &g_BirdSpell,			&player );
	HUD_PrepEntity( &g_BearBite,			&player );
	HUD_PrepEntity( &g_DragonBreathSpell,	&player );
	HUD_PrepEntity( &g_WyvernSpell,			&player );
	HUD_PrepEntity( &g_IcePokeSpell,		&player );
	HUD_PrepEntity( &g_FreezeRaySpell,		&player );
	HUD_PrepEntity( &g_BeanStalkSpell,		&player );
	HUD_PrepEntity( &g_ThornBlastSpell,		&player );
	HUD_PrepEntity( &g_ComboSpell,			&player );
}
...
void HUD_WeaponsPostThink( local_state_s *from, local_state_s *to, usercmd_t *cmd, double time, unsigned int random_seed )
{
	...
	// XYPHN - 040301
	//	Can it really be this simple? Only updating the time when g_runfuncs is
	//	true seems to fix all the client side problems we have been having. The
	//	only problem now seems to be the client skipping a fire because of
	//	packet loss, which is acceptable.
	if( g_runfuncs )
		gpGlobals->time = time; // Get current clock

	// isn't this easier then maintaining a switch list?
	for ( i = 0; i < 32; i++ )
	{
		if( !g_pWpns[i] )
			continue;

		if( g_pWpns[i]->m_iId == from->client.m_iId )
		{
			pWeapon = g_pWpns[i];
			break;
		}
	...
	// handle weapon changes that happened on the server
	if( pWeapon != player.m_pActiveItem )
	{
...
//		pCurrent->m_flPumpTime			= pfrom->m_flPumpTime;
	...
	player.pev->waterlevel = from->client.waterlevel; //LRC - for DMC fog
	...
	player.ammo_mana		= (int)from->client.ammo_nails; //is an int anyways...
	player.ammo_satchels	= (int)from->client.ammo_shells; 
	...
	to->client.ammo_nails				= player.ammo_mana;
	to->client.ammo_shells				= player.ammo_satchels;
	...
	if ( g_runfuncs && ( HUD_GetWeaponAnim() != to->client.weaponanim ) )
	{
		// Force a fixed anim down to viewmodel
		 HUD_SendWeaponAnim( to->client.weaponanim, 0, 1 );
	}
