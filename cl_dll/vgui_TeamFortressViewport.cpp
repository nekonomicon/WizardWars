...
#define SBOARD_INDENT_X			XRES(40)
#define SBOARD_INDENT_Y			YRES(40)

// low-res scoreboard indents
#define SBOARD_INDENT_X_512		10
#define SBOARD_INDENT_Y_512		30
...
TeamFortressViewport::TeamFortressViewport(int x,int y,int wide,int tall) : Panel(x,y,wide,tall), m_SchemeManager(wide,tall)
{
	...
	SchemeHandle_t hWindowScheme = m_SchemeManager.getSchemeHandle( "Window" );
	{
		// background color (transparent black)
		m_SchemeManager.getBgColor( hWindowScheme, r, g, b, a );
		pScheme->setColor(Scheme::sc_primary3, r, g, b, a );

		// used for orange borders around buttons
		m_SchemeManager.getBorderColor( hWindowScheme, r, g, b, a );
	...
	SchemeHandle_t hPrimaryScheme = m_SchemeManager.getSchemeHandle( "Primary Button Text" );
	{
		...
		// pScheme->setColor(Scheme::sc_secondary1, r, g, b, a );
		//pScheme->setColor(Scheme::sc_secondary1, 200*0.7, 200*0.7, 255*0.7, 0);
	}
	...
	// XYPHN - 040125
	//  new custom hud
	m_pCustomHUD = new CustomHUD();
	m_pCustomHUD->setParent( this );
	m_pCustomHUD->Init();
...
//		pMenu->AddButton( m_pTeamButtons[5] ); 
	...
	else if ( !strcmp( pButtonName, "!CHANGECLASS" ) )
	{
		...
		for (int i = PC_UNDEFINED + 1; i <= PC_RANDOM; i++ )
		{
			char sz[256]; 
	...
	else if ( !strcmp( pButtonName, "!SPECTATE" ) )
	{
		// Spectate button
		pButton = new SpectateButton( CHudTextMessage::BufferedLocaliseTextString( "#Menu_Spectate" ), 0, BUTTON_SIZE_Y, CMENU_SIZE_X, BUTTON_SIZE_Y, false);
		pButton->addActionSignal(new CMenuHandler_StringCommand( "spectate" ));
	}
	// Map Briefing
...
//		pButton = new ClassButton(0, pButtonText, 0, BUTTON_SIZE_Y * m_pCurrentCommandMenu->GetNumButtons(), CMENU_SIZE_X, BUTTON_SIZE_Y, false);
		pButton = new ClassButton(-1, pButtonText, 0, BUTTON_SIZE_Y * m_pCurrentCommandMenu->GetNumButtons(), CMENU_SIZE_X, BUTTON_SIZE_Y, false);
	...
	else if ( !strcmp( pButtonName, "!HELP" ) )
	{
		pButton = new ClassButton(0, pButtonText, 0, BUTTON_SIZE_Y * m_pCurrentCommandMenu->GetNumButtons(), CMENU_SIZE_X, BUTTON_SIZE_Y, false);
		pButton->addActionSignal(new CMenuHandler_TextWindow(MENU_HELP));
		// Create an input signal that'll popup the current menu
		pButton->addInputSignal( new CMenuHandler_PopupSubMenuInput(pButton, m_pCurrentCommandMenu) );
	}
	return pButton;
...
void TeamFortressViewport::InputPlayerSpecial( void )
{
	...
	if( gEngfuncs.Cmd_Argc() > 1 )
	{
		...
		sprintf(buffer,"_special %s\n",gEngfuncs.Cmd_Argv(1));
		ClientCmd(buffer);
	}
	else if(g_iPlayerClass==LIGHTNING_CLASS)
	{
		if(cl_doublezoom->value)
		{
			if( gHUD.m_iFOV > 25 || gHUD.m_iFOV == 0 )
				g_lastFOV = 25;
			else if( gHUD.m_iFOV > 15 )
				g_lastFOV = 15;
			else
				g_lastFOV = 90;
		}
	...
	else if(g_iPlayerClass == NATURE_CLASS			||
/*			g_iPlayerClass == FIRE_CLASS			||*/
			g_iPlayerClass == LIFE_CLASS			||
			g_iPlayerClass == ICE_CLASS				||
			g_iPlayerClass == DRAGON_CLASS)
	{
...
void TeamFortressViewport::UpdateSpectatorPanel()
{
	...
	if ( g_iUser1 && gHUD.m_pCvarDraw->value && !gHUD.m_iIntermission)	// don't draw in dev_overview mode
	{
		...
		int timer = (int)( gHUD.m_flRoundEnd - gHUD.m_flTime );
...
CMenuPanel* TeamFortressViewport::CreateTextWindow( int iTextToShow )
{
	...
	{
		if(g_iPlayerClass < 1 || g_iPlayerClass >= PC_RANDOM)
			return NULL;

		sprintf(sz, "#Help_%s", sTFClassSelection[g_iPlayerClass]);
		cText = CHudTextMessage::BufferedLocaliseTextString( sz );

		sprintf(sz, "%s", sLocalisedClasses[g_iPlayerClass]);
		CHudTextMessage::LocaliseTextString( sz, cTitle, MAX_TITLE_LENGTH );
...
// WIZWARS
	else if ( iTextToShow == SHOW_HELP )
	{
		...
		strncpy(cTitle,"Help",MAX_TITLE_LENGTH);
		cTitle[MAX_TITLE_LENGTH-1] = 0;
	}
...
// WIZWARS
	case MENU_WORLD:
		pNewMenu=ShowWorldPanel();
		break;
	case MENU_KICK:
		pNewMenu = ShowKickMenu();
		break;
	case MENU_HELP:
		pNewMenu = CreateTextWindow( SHOW_HELP );
		break;
...
bool TeamFortressViewport::SlotInput( int iSlot )
{
	...
	if ( m_pCurrentMenu && m_pCurrentMenu->SlotInput( iSlot ) )
		return TRUE;
...
int TeamFortressViewport::MsgFunc_ValTeams( const char * pszName, int iSize, void * pbuf )
{
	BEGIN_READ( pbuf, iSize );

	m_iNumberOfTeams	= READ_BYTE();
	m_iValidTeams		= READ_BYTE();
...
int TeamFortressViewport::MsgFunc_VGUIMenu(const char *pszName, int iSize, void *pbuf )
{
	...
	if(iMenu==MENU_CLASS)
	{
//		g_iMenuTeamNumber = READ_BYTE();
		g_iTeamNumber = READ_BYTE();
	}
...
int TeamFortressViewport::MsgFunc_ServerName( const char *pszName, int iSize, void *pbuf )
{
	...
	memset( m_szServerName, 0, MAX_SERVERNAME_LENGTH );
...
// Codiac 1.2 B20 TeamScore Fix
	...
	g_TeamInfo[iTeam].frags = READ_SHORT();
	g_TeamInfo[iTeam].scores_overriden = TRUE;
...
//		strncpy( g_PlayerExtraInfo[cl].teamname, READ_STRING(), MAX_TEAM_NAME );
		strncpy( g_PlayerExtraInfo[cl].teamname, m_sTeamNames[READ_BYTE()], MAX_TEAM_NAME );
	}
...
CMenuPanel* TeamFortressViewport::ShowKickMenu()
{
	// Don't open menus in demo playback
	if ( gEngfuncs.pDemoAPI->IsPlayingback() )
		return NULL;
...
void TeamFortressViewport::CreateKickMenu()
{
	// Create the panel
	m_pKickMenu = new CKickMenu(100, false, 0, 0, ScreenWidth, ScreenHeight);
	m_pKickMenu->setParent(this);
	m_pKickMenu->setVisible( false );
}
...
// WORLD PANEL
//======================================================================================
// Bring up the World Panel
CMenuPanel* TeamFortressViewport::ShowWorldPanel()
{
	// Don't open menus in demo playback
	if ( gEngfuncs.pDemoAPI->IsPlayingback() )
		return NULL;
	...
	return m_pWorldPanel;
}

void TeamFortressViewport::CreateWorldPanel()
{
	// Create the panel
	m_pWorldPanel = new CWorldPanel(100, false, 0, 0, ScreenWidth, ScreenHeight);
	m_pWorldPanel->setParent(this);
	m_pWorldPanel->setVisible( false );
}
