...
{
	{ NULL,			10,			Label::a_east },		// blank column to take up the slack
	{ NULL,			200,		Label::a_center },		// name
	{ NULL,			90,			Label::a_center },		// class
	{ NULL,			30,			Label::a_center },		// score
	{ NULL,			30,			Label::a_center },		// deaths
	{ NULL,			50,			Label::a_east },		// latency
	{ NULL,			20,			Label::a_center },		// seal
	{ NULL,			20,			Label::a_east },		// voice
...
ScorePanel::ScorePanel(int x,int y,int wide,int tall) : Panel(x,y,wide,tall)
{
	...
	LineBorder *border = new LineBorder( Color( r, g, b, a ) );
	...
	m_TitleLabel.setBounds(xpos, 4, wide - 100, SBOARD_TITLE_SIZE_Y);
	m_TitleLabel.setContentFitted(false);
	m_TitleLabel.setParent(this);

	m_TimeLabel.setFont(tfont);
	m_TimeLabel.setText("");
	m_TimeLabel.setBgColor( 0, 0, 0, 255 );
	m_TimeLabel.setFgColor( Scheme::sc_primary1 );
	m_TimeLabel.setContentAlignment( vgui::Label::a_east );
	m_TimeLabel.setBounds( wide - 100, 4, 87, SBOARD_TITLE_SIZE_Y);
	m_TimeLabel.setContentFitted(false);
	m_TimeLabel.setParent(this);

	// XYPHN - 040202
	//	Icons instead of text
	m_pScoreImage = vgui_LoadTGA( "gfx/vgui/scoreicon.tga" );
	m_pDeathImage = vgui_LoadTGA( "gfx/vgui/deathicon.tga" );
	m_pNetImage   = vgui_LoadTGA( "gfx/vgui/neticon.tga"   );
	...
	for(int i=0; i < NUM_COLUMNS; i++)
	{
		...
		case COLUMN_KILLS:
			m_HeaderLabels[i].setImage( m_pScoreImage );
			break;

		case COLUMN_DEATHS:
			m_HeaderLabels[i].setImage( m_pDeathImage );
			break;

		case COLUMN_LATENCY:
			m_HeaderLabels[i].setImage( m_pNetImage );
			break;
		}
...
ScorePanel::~ScorePanel()
{
	if( m_pScoreImage )
	{
		delete m_pScoreImage;
		m_pScoreImage = NULL;
	}
	
	if( m_pDeathImage )
	{
		delete m_pDeathImage;
		m_pDeathImage = NULL;
	}
...
void ScorePanel::UpdateTime( void )
{
	...
	char sz[64];
	sprintf( sz, "%d:%02d", (int)( timer / 60.0f ), (int)timer % 60 );

	m_TimeLabel.setText( sz );
}


void ScorePanel::SortXyphn( void )
{
	m_iRows = 0;
	...
	for( int i = 1; i <= 6; i++ )
	{
		...
		if ( !g_TeamInfo[team].scores_overriden )
			g_TeamInfo[team].frags = g_TeamInfo[team].deaths = 0;

		g_TeamInfo[team].ping = g_TeamInfo[team].packetloss = 0;
		g_TeamInfo[team].players = 0;
		...
		for( int j = 1; j < MAX_PLAYERS; j++ )
		{
			...
			if( team != 5 && g_IsSpectator[j] )
				continue;

			if( team == 0 && g_PlayerExtraInfo[j].teamnumber != 0 )
				continue;

			if( team == 5 && !g_IsSpectator[j] )
				continue;

//			if( team == 5 && g_PlayerExtraInfo[j].teamnumber != 0 )
//				continue;

			if( g_PlayerExtraInfo[j].teamnumber != team && team != 5 && team != 0 )
				continue;

			g_TeamInfo[team].players++;

			if( !g_TeamInfo[team].scores_overriden )
			{
				g_TeamInfo[team].frags  += g_PlayerExtraInfo[j].frags;
				g_TeamInfo[team].deaths += g_PlayerExtraInfo[j].deaths;
			}

			g_TeamInfo[team].ping += g_PlayerInfoList[j].ping;
			g_TeamInfo[team].packetloss += g_PlayerInfoList[j].packetloss;

			if( g_PlayerInfoList[j].thisplayer )
				g_TeamInfo[team].ownteam = TRUE;
			else
				g_TeamInfo[team].ownteam = FALSE;

			g_TeamInfo[team].teamnumber = i;

			if( !bTeam )
			{
				bTeam = true;
				m_iSortedRows[ m_iRows ] = i;
				m_bHasBeenSorted[ m_iRows ] = true;
		...
		g_TeamInfo[team].already_drawn = FALSE;

		if ( g_TeamInfo[team].players > 0 )
		{
			g_TeamInfo[team].ping		/= g_TeamInfo[team].players;
			g_TeamInfo[team].packetloss	/= g_TeamInfo[team].players;
		}
	}
}
...
void ScorePanel::FillGrid()
{
	...
	for(int row=0; row < NUM_ROWS; row++)
	{
		...
		for(int col=0; col < NUM_COLUMNS; col++)
		{
			...
			else if (col == COLUMN_TRACKER || col == COLUMN_KILLS || col == COLUMN_DEATHS )
			{
			...
			else
			{
				...
				case COLUMN_CLASS:
					...
					if ( g_iTeamNumber == 0 && gHUD.m_Teamplay )
						bShowClass = false;
					...
					if (bShowClass)
					{
						...
						{
							if ( gViewPort->GetValidClasses( g_PlayerExtraInfo[ m_iSortedRows[row] ].teamnumber ) == WW_ILL_ARCHMAGE )
								bNoClass = false;
							else if ( gViewPort->GetValidClasses( g_PlayerExtraInfo[ m_iSortedRows[row] ].teamnumber ) != -1 )
								bNoClass = true;
						}
