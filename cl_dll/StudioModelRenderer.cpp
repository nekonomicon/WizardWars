...
int CStudioModelRenderer::StudioDrawModel( int flags )
{
	...
	if (flags & STUDIO_RENDER)
	{
		...
		if( m_pCurrentEntity->curstate.team == 123 && m_pCurrentEntity->curstate.rendermode != 0 )
		{
			lighting.color.x = m_pCurrentEntity->curstate.renderamt / 256.0f;
			lighting.color.y = m_pCurrentEntity->curstate.renderamt / 256.0f;
			lighting.color.z = m_pCurrentEntity->curstate.renderamt / 256.0f;
			lighting.shadelight = 0;
			lighting.ambientlight = 64;
		}
...
/*
	// XYPHN - 040225
	//	We don't use the "model" infobuffer to get model anymore
	m_pRenderModel = IEngineStudio.SetupPlayerModel( m_nPlayerIndex );
	if (m_pRenderModel == NULL)
		return 0;
*/
	// XYPHN - 040225
	//	This tells it to use the current model the server set
	m_pRenderModel = m_pCurrentEntity->model;
...
void CStudioModelRenderer::StudioRenderModel( void )
{
	...
	else if( m_pCurrentEntity->curstate.renderfx == kRenderFxFrosted )
	{
		m_pCurrentEntity->curstate.renderfx = kRenderFxNone;
		StudioRenderFinal();
		...
		IEngineStudio.SetForceFaceFlags( STUDIO_NF_FLATSHADE | STUDIO_NF_CHROME | STUDIO_NF_FULLBRIGHT );

		int iSprite = SPR_Load( "sprites/xspark1.spr" );

		model_t * pModel = (struct model_s *)gEngfuncs.GetSpritePointer( iSprite );
		...
		m_pCurrentEntity->curstate.renderfx = kRenderFxFrosted;
