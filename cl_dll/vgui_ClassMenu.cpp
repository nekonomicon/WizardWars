...
CClassMenuPanel::CClassMenuPanel(int iTrans, int iRemoveMe, int x,int y,int wide,int tall) : CMenuPanel(iTrans, iRemoveMe, x,y,wide,tall)
{
	...
	m_pScrollPanel->setBorder( new LineBorder( Color( r, g, b, a ) ) );
	...
	for (int i = WIND_CLASS; i < PC_LASTCLASS; i++)
	{
		char sz[256]; 
...
//		ActionSignal *pASignal = new CMenuHandler_StringCommandClassSelect( sTFClassSelection[i], true );
		...
		sprintf(sz, "%s", sLocalisedClasses[i]);
...
{
	// let players select a class in deathmatch
	if ( !g_iTeamNumber && gHUD.m_Teamplay)
		return;

	// Don't allow the player to join a team if they're not in a team
	if( g_iTeamNumber < 0 || g_iTeamNumber > 4 )
		return;
	...
	for( int i = WIND_CLASS; i < PC_LASTCLASS; i++ )
	{
		...
		if ( gViewPort->GetValidClasses( g_iTeamNumber ) == -1 ||
			 gViewPort->GetValidClasses( g_iTeamNumber ) == WW_ILL_ARCHMAGE )
		{
			if( i == ARCHMAGE_CLASS )
			{
				m_pButtons[ ARCHMAGE_CLASS ]->setVisible( true );
				m_pButtons[ ARCHMAGE_CLASS ]->setPos( CLASSMENU_TOPLEFT_BUTTON_X, iYPos );
				SetActiveInfo( ARCHMAGE_CLASS );
		...
		{
			if ( i == ARCHMAGE_CLASS || m_pButtons[i]->IsNotValid() )
			{
				m_pButtons[i]->setVisible( false );
			}
			else
		...
		for ( int j = 1; j < MAX_PLAYERS; j++ )
		{
			...
			if ( g_PlayerExtraInfo[j].playerclass != i)//Codiac && !bCivilian )
				continue;
		...
		for ( int team = 0; team < MAX_TEAMS; team++ )
		{
			...
			else if ( m_pClassImages[0][i] != NULL )
			{
...
bool CClassMenuPanel::SlotInput( int iSlot )
{
	...
	if (iSlot == 0)
	{
		if( gViewPort->GetValidClasses( g_iTeamNumber ) == -1 ||
			gViewPort->GetValidClasses( g_iTeamNumber ) == WW_ILL_ARCHMAGE )
		{
			m_pButtons[ ARCHMAGE_CLASS ]->fireActionSignal();
			return true;
		}
...
void CClassMenuPanel::SetActiveInfo( int iInput )
{
	...
	if ( iInput > PC_RANDOM || iInput < WIND_CLASS )
		iInput = 0;
