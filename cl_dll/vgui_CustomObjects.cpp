...
{
	// XYPHN - 030704 - w00t! 4th of July!
	//  Apparently we still want TFC compatibility
	//  and it's whacking backwardsnes ;)

	if( gHUD.m_Teamplay && g_iTeamNumber == 0 )
		return true;

	if( m_iPlayerClass < WIND_CLASS || m_iPlayerClass >= PC_LASTCLASS )
		return false;

	if( gViewPort->GetValidClasses( g_iTeamNumber ) == 0 && m_iPlayerClass != ARCHMAGE_CLASS )
		return false;

	if( gViewPort->GetValidClasses( g_iTeamNumber ) == -1 && m_iPlayerClass != ARCHMAGE_CLASS )
		  return true;
	...
	if( gViewPort->GetValidClasses( g_iTeamNumber ) & sTFValidClassInts[ m_iPlayerClass ] )
		return true;
