...
void CHalfLifeMultiplay :: InitHUD( CBasePlayer *pl )
{
	...
	if ( g_teamplay )
	{
		UTIL_LogPrintf( "\"%s<%i><%s><%s>\" entered the game\n",  
			...
			g_pTFGlobalInfo->TeamName( pl->pev->team ) );
	}
	else
	{
...
void CHalfLifeMultiplay :: ClientDisconnected( edict_t *pClient )
{
		...
		return;
			...
			if ( g_teamplay )
			{
				UTIL_LogPrintf( "\"%s<%i><%s><%s>\" disconnected\n",  
					...
					g_pTFGlobalInfo->TeamName( pPlayer->pev->team ) );
			}
			else
			{
	...
	while( ( client = (CBasePlayer*)UTIL_FindEntityByClassname( client, "player" ) ) != NULL ) 
		...
		if( (int)client->m_hObserverTarget == NULL )
			continue;
...
void CHalfLifeMultiplay :: PlayerKilled( CBasePlayer *pVictim, entvars_t *pKiller, entvars_t *pInflictor )
{
	...
	if ( pVictim->pev == pKiller || pVictim == ktmp )  
	{  // killed self
		pKiller->frags -= 1;
	}
	...
	MESSAGE_BEGIN( MSG_ALL, gmsgScoreInfo );
		...
		WRITE_BYTE ( pVictim->pev->playerclass );
		WRITE_BYTE ( pVictim->pev->team );
	MESSAGE_END();
	...
	if ( ep && ep->Classify() == CLASS_PLAYER )
	{
		...
		MESSAGE_BEGIN( MSG_ALL, gmsgScoreInfo );
			...
			WRITE_BYTE ( PK->pev->playerclass );
			WRITE_BYTE ( PK->pev->team );
		MESSAGE_END();
...
//	char *gluon = "gluon gun";
	...
	else if( strncmp( killer_weapon_name, "ww_spell_", 9 ) == 0 )
		killer_weapon_name += 9;
	else if( strncmp( killer_weapon_name, "ww_", 3 ) == 0 )
		killer_weapon_name += 3;
	else if( strncmp( killer_weapon_name, "monster_", 8 ) == 0 )
		killer_weapon_name += 8;
	else if( strncmp( killer_weapon_name, "func_", 5 ) == 0 )
		killer_weapon_name += 5;
	else if( strncmp( killer_weapon_name, "proj_", 5 ) == 0 )
		killer_weapon_name += 5;
	...
	// replace the full names with small names
	if( strncmp( killer_weapon_name, "seal", 4 ) == 0 )
		killer_weapon_name = "seal";
	if( strncmp( killer_weapon_name, "satchel", 7 ) == 0 )
		killer_weapon_name = "satchel";
	...
	if ( pVictim->pev == pKiller )  
	{
		...
		if ( g_teamplay )
		{
			UTIL_LogPrintf( "\"%s<%i><%s><%s>\" committed suicide with \"%s\"\n",  
				...
				g_pTFGlobalInfo->TeamName( pVictim->pev->team ),
				killer_weapon_name );		
		}
		else
		{
	...
	else if ( pKiller->flags & FL_CLIENT )
	{
		...
		if ( g_teamplay )
		{
			UTIL_LogPrintf( "\"%s<%i><%s><%s>\" killed \"%s<%i><%s><%s>\" with \"%s\"\n",  
				...
				g_pTFGlobalInfo->TeamName( pKiller->team ),
				STRING( pVictim->pev->netname ),
				GETPLAYERUSERID( pVictim->edict() ),
				GETPLAYERAUTHID( pVictim->edict() ),
				g_pTFGlobalInfo->TeamName( pVictim->pev->team ),
...
void CHalfLifeMultiplay :: GoToIntermission( void )
{
	...
	//	trepid_jon - 040523
	//	Apparently this code would make entities disappear after map changing
	//	when you would go into intermission before the map changed
	//	I don't think we should do this anyway, since everyone's used to
	//	seeing what they were seeing right when the map changed
	//	like having a sniper dot on an enemy's head

/*
	// XYPHN - 040228
	//	Make everyone change to the info_player_start view on intermission
	...
	if( pStart != NULL )
	{
		...
		for( i = 1; i <= gpGlobals->maxClients; i++ )
		{
			...
			if( pPlayer != NULL )
				SET_VIEW( pPlayer->edict(), pStart->edict() );
		}
	}
*/
	g_fGameOver = TRUE;
...
void CHalfLifeMultiplay :: ChangeLevel( void )
{
	...
	MESSAGE_BEGIN(MSG_ALL,gmsgMusic,NULL);
		WRITE_STRING(MUSIC_STOP);
	MESSAGE_END();
