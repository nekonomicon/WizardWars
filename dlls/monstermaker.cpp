...
LINK_ENTITY_TO_CLASS( ww_monstermaker, CMonsterMaker );
...
void CMonsterMaker :: KeyValue( KeyValueData *pkvd )
{
	...
	if(FStrEq(pkvd->szKeyName,"iuser2")){
		pev->iuser2 = atoi(pkvd->szValue);
		pkvd->fHandled=TRUE;
	}
	if(FStrEq(pkvd->szKeyName,"iuser3")){
		pev->iuser3=atoi(pkvd->szValue);
		pkvd->fHandled=TRUE;
	}
	if(FStrEq(pkvd->szKeyName,"iuser4")){
...
void CMonsterMaker::MakeMonster( void )
{
	...
	//WIZWARS:Copy over user vars
	pevCreate->team		= pev->team;
	pevCreate->iuser1	= pev->iuser1;
	pevCreate->iuser2	= pev->iuser2;
	pevCreate->iuser3	= pev->iuser3;
	pevCreate->iuser4	= pev->iuser4;
