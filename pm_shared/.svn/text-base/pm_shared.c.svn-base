...
#define PLAYER_MAX_SAFE_FALL_SPEED	900//580// approx 20 feet
#define DAMAGE_FOR_FALL_SPEED		(float) 100 / ( PLAYER_FATAL_FALL_SPEED - PLAYER_MAX_SAFE_FALL_SPEED )// damage per unit per second.
#define PLAYER_MIN_BOUNCE_SPEED		200
#define PLAYER_FALL_PUNCH_THRESHHOLD (float)450//350 // won't punch player's screen/make scrape noise unless player falling at least this fast.
...
//		wishspd = 30;
	...
	if(atoi(pmove->PM_Info_ValueForKey(pmove->physinfo,"dragon"))){
		vec3_t forward;
		vec3_t right;
		vec3_t up;

		AngleVectors(pmove->angles,forward,right,up);
		if(DotProduct(wishdir,forward)<0)
			maxwishspd=200;
		else
			maxwishspd=600;
	}
	...
	{
		if(atoi(pmove->PM_Info_ValueForKey(pmove->physinfo,"windwizard")))
			maxwishspd=100;
	}

	if (wishspd > maxwishspd)
		wishspd = maxwishspd;
...
void PM_AirMove (void)
{
	...
	vec3_t		wishdir;
	float		wishspeed;
	int			dragon=atoi(pmove->PM_Info_ValueForKey(pmove->physinfo,"dragon"));
...
void PM_LadderMove( physent_t *pLadder )
{
	...
	{
		VectorCopy( pLadder->mins, modelmins );
		VectorCopy( pLadder->maxs, modelmaxs );
	}
	...
	else
	{
		...
		trace2=pmove->PM_PlayerTrace(pmove->origin,pLadder->origin,PM_NORMAL,-1);

		trace.fraction=trace2.fraction;
		VectorCopy(trace2.plane.normal,trace.plane.normal);
	}
...
physent_t *PM_Ladder( void )
{
	...
	//WIZWARS:BEANSTALK look for physents
	for( i = 0; i < pmove->numphysent; i++ )
	{
		...
		if( pe->frame == 254 )
		{
			...
			VectorCopy(pe->origin,end);

			VectorSubtract(end,pmove->origin,end);

			if( Length(end) < 60 )
			{
				VectorNormalize(end);
				VectorScale(end,10,end);
				VectorAdd(pmove->origin,end,end);
...
void PM_Jump (void)
{
	...
	bear	= atoi( pmove->PM_Info_ValueForKey( pmove->physinfo, "bear" ) ) == 1 ? true : false;
	dragon	= atoi( pmove->PM_Info_ValueForKey(pmove->physinfo, "dragon" ) ) == 1 ? true : false;//WIZWARS:Dragon
	...
	if( bear )
		pmove->PM_PlaySound( CHAN_BODY, "player/bear_jump.wav", 1.0f, ATTN_NORM, 0, PITCH_NORM );
...
void PM_CheckFalling( void )
{
		...
		pmove->flFallVelocity >= PLAYER_FALL_PUNCH_THRESHHOLD )
	{
		...
		else if( pmove->flFallVelocity > PLAYER_MAX_SAFE_FALL_SPEED )
		{
			...
			{
			case 0:
				pmove->PM_PlaySound( CHAN_VOICE, "player/fall_pain1.wav", 1, ATTN_NORM, 0, PITCH_NORM );
				break;
			case 1:
				pmove->PM_PlaySound( CHAN_VOICE, "player/fall_pain2.wav", 1, ATTN_NORM, 0, PITCH_NORM );
				break;
			case 2:
				pmove->PM_PlaySound( CHAN_VOICE, "player/fall_pain3.wav", 1, ATTN_NORM, 0, PITCH_NORM );
				break;
			}
...
vec3_t origins[MAX_CLIENTS][MAX_ORIGINS];
int	iDragonCounter[MAX_CLIENTS];

void PM_DragonCollision(vec3_t start, vec3_t end){
	pmtrace_t trace;
	vec3_t temp;
	...
	if(trace.fraction<0.99){
		float dist=100*(1-trace.fraction);

		VectorSubtract(start,end,temp);
		VectorNormalize(temp);
		VectorScale(temp,dist,temp);

		if(Length(temp)>60){
			VectorCopy(origins[pmove->player_index][2],pmove->origin);
		}
		else
			VectorAdd(pmove->origin,temp,pmove->origin);
...
void PM_DragonRoll(){
	...
	VectorCopy(pmove->velocity,temp2);
	temp2[2]=0;

	VectorCopy(pmove->origin,temp);
	VectorSubtract(origins[pmove->player_index][MAX_ORIGINS-1],temp,temp);
	temp[2]=0;

	VectorNormalize(temp);
	VectorNormalize(temp2);

	val=DotProduct(temp2,temp);
	val=val*val*val;
	...
	temp[2]=0;
	VectorNormalize(temp);
	newRoll*=DotProduct(temp,forward);
	newRoll*=Length(pmove->velocity)/200;
	VectorCopy(pmove->origin,temp);
	VectorSubtract(origins[pmove->player_index][MAX_ORIGINS-1],temp,temp);
	if(DotProduct(temp,pmove->velocity)>0)
		newRoll=0;

	pmove->angles[ROLL]+=newRoll;
	pmove->angles[ROLL]/=2.0;

	if(iDragonCounter[pmove->player_index] > MAX_ORIGINS)
	{
// Codiac - BUGBUG
...
void PM_PlayerMove( qboolean server )
{
	...
	int dragon=atoi(pmove->PM_Info_ValueForKey(pmove->physinfo,"dragon"));//WIZWARS:Dragon
	...
	if(dragon){
		...
		iDragonCounter[pmove->player_index]++;

		if(iDragonCounter[pmove->player_index]==1){
			pmove->flTimeStepSound=200;

			for(x=0;x<MAX_ORIGINS;x++){
				VectorCopy(pmove->origin,origins[pmove->player_index][x]);
			}
		}

		VectorCopy(pmove->origin,temp);
		temp[2] -= 60;
		PM_DragonCollision(pmove->origin,temp);
		temp[2] += 30;
		temp[1] += 60;
		PM_DragonCollision(pmove->origin,temp);
		temp[1] -= 120;
		...
		{
			pmove->velocity[2] += 20;
			if(pmove->velocity[2] > 200)
				pmove->velocity[2] = 200;
		}

		VectorScale(pmove->velocity, .95, pmove->velocity);

#ifdef CLIENT_DLL
		PM_DragonRoll();
#endif
		for(x = MAX_ORIGINS - 2; x >= 0; x--)
			VectorCopy(origins[pmove->player_index][x],origins[pmove->player_index][x+1]);
		VectorCopy(pmove->origin,origins[pmove->player_index][0]);
		...
		iDragonCounter[pmove->player_index] = 0;
